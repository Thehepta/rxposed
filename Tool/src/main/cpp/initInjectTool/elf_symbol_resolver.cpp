...
static std::vector<RuntimeModule> & getProcessMapWithProcMaps(pid_t pid){
    if (modules == nullptr) {
...
RuntimeModule GetProcessMaps(pid_t pid,const char *name) {
    auto modules = getProcessMapWithProcMaps(pid);
...
void *ResolveElfInternalSymbol( pid_t pid,const char *library_name, const char *symbol_name) {
    ...
    RuntimeModule module = GetProcessMaps(pid,library_name);
