...
std::vector<MapInfo> MapInfo::Scan(const std::string& pid) {
    ...
    if (maps) {
        ...
        while ((read = getline(&line, &len, maps.get())) > 0) {
            ...
            auto ref = MapInfo{start, end, 0, perm[3] == 'p', off, static_cast<dev_t>(makedev(dev_major, dev_minor)),inode, line + path_off};
...
ssize_t read_proc(pid_t pid, uintptr_t remote_addr, void *buf, size_t len) {
    struct iovec local{
            .iov_base = (void *) buf,
            .iov_len = len
    };
