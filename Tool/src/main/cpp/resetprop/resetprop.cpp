...
static void (*system_property_read_callback)(const prop_info*, void (*)(void*, const char*, const char*, uint32_t), void*);
...
static int set_prop(const char *name, const char *value, PropFlags flags) {
    ...
    if (ret == 0 && flags.isSkipSvc() && flags.isPersist() && str_starts(name, "persist.")) {
        LOGD("resetprop:  not support persist\n");
//        ret = persist_set_prop(name, value) ? 0 : 1;
    }

    if (ret) {
        LOGD("resetprop: set prop error\n");
    }
